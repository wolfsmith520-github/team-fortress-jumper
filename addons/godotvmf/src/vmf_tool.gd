...
static func create_mesh(vmf_structure: VMFStructure, offset: Vector3 = Vector3.ZERO, optimized: bool = true) -> ArrayMesh:
	...
	for sides in material_sides.values():
		...
		for side: VMFSide in sides:
			...
			# Micky: Isolating the issue.
			#if not side.material == "nature/blendrockgroundwallforest":
				#continue
			#if side.is_displacement:
				#continue
			...
			if not side.is_displacement:
				...
				#print("VMFTOOL", side) # Micky: Debugging.
				#print(side.dispinfo.vertices.size(), "  ", verts_count) # Micky: Debugging.
				# Micky: Avoiding mesh corruption at all costs.
				if not side.dispinfo.vertices:
					VMFLogger.error("Displacement's vertices are empty, likely not assigned properly in its constructor.");
					continue;
		...
		# Micky: Crash in ItemTest.
		if optimized: sf.optimize_indices_for_cache();
		# Micky: Error in ItemTest.
		# ERROR: scene/resources/surface_tool.cpp:782 - Condition "uint32_t(index) >= old_vertex_array.size()" is true.
		sf.generate_normals();
